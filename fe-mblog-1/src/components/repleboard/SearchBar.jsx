...
const SearchBar = () => {
  ...
  const [tTitle, setTTitle] = useState("제목"); //제목, 내용, 작성자 중에서 한가지
  ...
  useEffect(() => {
    ...
    search.split("&").forEach((item) => {
      ...
      //요청 url에 담긴 condition 정보를
      if (item.match("condition")) {
        setTTitle(item.split("=")[1]);
      }
    });
  }, [search, setTTitle]); //의존 배열에 search를 사용했고 상태훅을 선택했으니 그 정보가 변경될때마다 호출

  const setPath = () => {
    console.log(tTitle, content);
    console.log(search);
    console.log(search.match("condition")); //자바스크립트에서는 싱글 혹은 더블로 묶지 않으면 변수취급함
    let path;
